...
export default {
  setup () {
    ...
    return {
        ...
        nombreRef,
        nombreRules: [
          val => (val !== null && val !== '') || 'introduzca su Nombre completo',
        ...
        async onSubmit () {
          ...
          if (emailRef.value.hasError || passwordRef.value.hasError || nombreRef.value.hasError) {
            $q.notify({
              color: 'negative',
            ...
            const resp = await newAccount({ user_nombre, user_email, user_password });
            if(resp.ok){
              $q.notify({
                color: 'secondary',
                message: resp.msg,
