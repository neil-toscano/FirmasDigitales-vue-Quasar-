      ...
      <q-card class="my-card">
        ...
        <q-card-section>
          ...
          <div class="wrap first-paragraph">
            ...
            <input  v-model="numeroRuc" placeholder="Número de Ruc si es que lo tiene" />
              
            <div class="text-h7">. {{ dd.content[0].text[4].text }} </div>
            <input  v-model="nombreContratista" placeholder="Nombre del contratista" />
            ...
            <input  v-model="ubiDomicilioContratista" placeholder="ubicación del domicilio" />
            
            <div class="text-h7"> {{ dd.content[0].text[10].text }} </div>
            <input  v-model="nombreSolicitaServicio" placeholder="Nombre del que solicita el servicio" />
            ...
            <input  v-model="domicilioSolicitaServicio" placeholder="domicilio del que solicita el servicio" />
        ...
        <q-card-section>
          <div class="row wrap first-paragraph">
            <div class="text-h7">
              <span class="text-bold">{{ dd.content[1].text[0].text }}</span> {{ dd.content[1].text[1].text }} 
        ...
        <q-card-section>
          <div>
            <div class="wrap first-paragraph">
              <div class="text-h7">
                <span class="text-bold">{{ dd.content[2].text[0].text }}</span> {{dd.content[2].text[1].text}}
              </div>

              <q-list dense bordered padding class="rounded-borders q-my-md list-horario">
                <q-item>
                  <!-- { table: {
              body: [
                [
                    ...
                    <div  v-for="(item, index) in [0,1,2]" :key="index">
                      <div  v-for="(item2, index2) in [0,1,2]" :key="index2">
                        <div class="row">
                          <div class="col">
                            <q-item-section>
                              <span><q-badge rounded color="yellow" /> 
                                {{ dd.content[3].table.body[index][index2].text[0].text }} 
                                <input  v-model="dd.content[3].table.body[index][index2].text[1].text"/>
                                {{ dd.content[3].table.body[index][index2].text[2].text }} 
                                <input  v-model="dd.content[3].table.body[index][index2].text[3].text"/>
                              </span>
                            </q-item-section>
                          </div>
                        </div>
        ...
        <q-card-section>
          <div>
            <div class="wrap first-paragraph">
              <div class="text-h7 text-bold"> {{ dd.content[4].text[0].text }} </div>
              ...
              <input  v-model="nombreOrquesta" placeholder="Nombre de la Orquesta" />
        ...
        <q-card-section>
          ...
          <ol>
            <li class="q-my-md"> {{ dd.content[6].ol[0].text[0].text }} <input  v-model="nombreLugarTrabajo" placeholder="Lugar Trabajo" /> </li>
            <li class="q-my-md"> {{ dd.content[6].ol[1].text[0].text }} <input  v-model="DiaFechaTrabajo" placeholder="Dia y Fecha de Trabajo" /> </li>
            <li class="q-my-md"> {{ dd.content[6].ol[2].text[0].text }} <input  v-model="horaPresentacion" placeholder="Hora presentación" /> </li>
            <li class="q-my-md"> {{ dd.content[6].ol[3].text[0].text }} <input  v-model="numIntegrantes" placeholder="número Integrantes" /> </li>
            <li class="q-my-md"> {{ dd.content[6].ol[4].text[0].text }} <input  v-model="contactoContratante" placeholder="Contacto Contratante" /> </li>
            <li class="q-my-md"> {{ dd.content[6].ol[5].text[0].text }} <input  v-model="nombreDirector" placeholder="Nombre Director" /> </li>
          </ol>
        </q-card-section>
        <q-card-section>
        ...
        <div class="row">
          <div class="col">
            <div id="signature-pad" class="signature-pad">
              <div class="signature-pad--body">
                <canvas style="background-color: pink;" ref="canvasRef"  width=300 height=200></canvas>
              </div>
              <div class="signature-pad--footer">
                ...
                <div class="signature-pad--actions">
                  <div>
                    <button type="button" class="button clear" data-action="clear" @click="clear">Clear</button>
                  ...
                  <div>
                    ...
                    <button type="button" class="button save" data-action="save-svg" @click="abrir">Abrir en nuevo window</button>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="col">
            <div id="signature-pad" class="signature-pad">
              <div class="signature-pad--body">
                <canvas style="background-color: pink;" ref="canvasRef_Second"  width=300 height=200></canvas>
              </div>
              <div class="signature-pad--footer">
                <div class="description">Sign above</div>
    ...
    setup () {
      ...
      const dd = computed(() => {
        return {
          content: [
            ...
            {
              text:[
                { text: 'CLAUSULA PRIMERA (OBJETO DEL CONTRATO):', fontSize: 15, bold: true},
                { text: 'La Orquesta, haciendo uso de sus atribuciones y previo acuerdo de los socios integrantes' +
            ...
            {
              text:[
                { text: 'CLÁUSULA SEGUNDA FORMA DEL TRABAJO:', fontSize: 15, bold: true},
                { text: 'La Orquesta respetara el horario de trabajo de 11 horas por día y como cortesía ' + 
            ...
            {
              text: [
                { text: 'CLÁUSULA TERCERA: (DE LA COMPENSACIÓN ECONÓMICA):', fontSize: 15, bold: true},
                { text: 'El Señor (a) '},
            ...
            {
              ol: [
                {text: [ { text: 'Lugar de trabajo:'}, { text: `${nombreLugarTrabajo.value}` }]},
                {text: [ { text: 'Días y fechas de trabajo:'}, { text: `${DiaFechaTrabajo.value}` }]},
                {text: [ { text: 'Hora de presentación:'}, { text: `${horaPresentacion.value}` }]},
                {text: [ { text: 'Compuesto por su plana titular con:'}, { text: `${numIntegrantes.value}` }]},
                {text: [ { text: 'Contacto del Contratante:'}, { text: `${contactoContratante.value}` }]},
                {text: [ { text: 'Contacto del director:'}, { text: `${nombreDirector.value}` }]},
              ]
            },
            {
              text: [
                { text: 'CLÁUSULA QUINTA (ANTECEDENTES):', fontSize: 15, bold: true},
                { text: ' En caso de incumplimiento de parte de la Orquesta, se compromete a devolver' +
            ...
            {text: '\n\nCLÁUSULA SEXTA: PENALIDADES Y DEBERES DE LA ORQUESTA:', style: 'header'},
            {
              ol: [
            ...
            {text: '\n\nCLÁUSULA SÉPTIMA: PENALIDADES Y DERECHOS DE LA ORQUESTA:', style: 'header'},
            {
              ol: [
      ...
      const abrir = () => {
        ...
        pdfDoc.getBlob((blob) => {
          ...
          .then(response => {
            console.log('El archivo fue enviado al backend con éxito', response);
          })
          .catch(error => {
      ...
      const clear = () => {
        ...
        dd.value.content.push({
          columns: [
            ...
            { image: signaturePad_second.toDataURL()}
